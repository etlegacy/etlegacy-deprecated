	...
	if (skillLevelPoints)
	{
		...
		for (i = 0; i < level.numConnectedClients; i++)
		{
			...
			for (x = 0; x < SK_NUM_SKILLS; x++)
			{
				...
				G_SetPlayerSkill(g_entities[level.sortedClients[i]].client, (skillType_t)x);
				if (oldskill != g_entities[level.sortedClients[i]].client->sess.skill[x])
				{
					...
					G_UpgradeSkill(&g_entities[level.sortedClients[i]], (skillType_t)x);
				}
			}
		}
