...
static void PM_BeginWeaponReload(int weapon)
{
	...
	if (pm->ps->ammoclip[BG_FindAmmoForWeapon((weapon_t)weapon)] >= GetAmmoTableData(weapon)->maxclip)
	{
		return;
	}
...
static void PM_ReloadClip(int weapon)
{
	int ammoreserve = pm->ps->ammo[BG_FindAmmoForWeapon((weapon_t)weapon)];
	int ammoclip    = pm->ps->ammoclip[BG_FindClipForWeapon((weapon_t)weapon)];
	...
	if (pm->ps->weaponTime <= 0)
	{
		...
		if (reloadRequested)
		{
			if (pm->ps->ammo[ammoWeap])
			{
				...
				if (IS_AKIMBO_WEAPON(weapon))
				{
					if (pm->ps->ammoclip[BG_FindClipForWeapon((weapon_t)weaponTable[weapon].akimboSideram)] < GetAmmoTableData(BG_FindClipForWeapon((weapon_t)weaponTable[weapon].akimboSideram))->maxclip)
					{
						doReload = qtrue;
					}
				}
		...
		else if (autoreload)
		{
			if (!pm->ps->ammoclip[clipWeap] && pm->ps->ammo[ammoWeap])
			{
				if (IS_AKIMBO_WEAPON(weapon))
				{
					if (!pm->ps->ammoclip[BG_FindClipForWeapon((weapon_t)weaponTable[weapon].akimboSideram)])
					{
						doReload = qtrue;
					}
				}
...
void PM_WeaponUseAmmo(int wp, int amount)
{
	...
	{
		int takeweapon = BG_FindClipForWeapon((weapon_t)wp);

		if (IS_AKIMBO_WEAPON(wp))
		{
			if (!BG_AkimboFireSequence(wp, pm->ps->ammoclip[BG_FindClipForWeapon((weapon_t)wp)], pm->ps->ammoclip[BG_FindClipForWeapon((weapon_t)weaponTable[wp].akimboSideram)]))
			{
...
static void PM_Weapon(void)
{
	...
	{
		akimboFire = BG_AkimboFireSequence(pm->ps->weapon, pm->ps->ammoclip[BG_FindClipForWeapon((weapon_t)pm->ps->weapon)], pm->ps->ammoclip[BG_FindClipForWeapon((weapon_t)weaponTable[pm->ps->weapon].akimboSideram)]);
	}
	else
	{
		akimboFire = qfalse;
	...
	if (pm->ps->weapon)
	{
		...
		if (ammoNeeded > ammoAvailable)
		{
			...
			qboolean reloading = (qboolean)(ammoNeeded <= pm->ps->ammo[BG_FindAmmoForWeapon((weapon_t)pm->ps->weapon)]);
	...
	case WP_AKIMBO_SILENCEDLUGER:
		...
		else if (!pm->ps->ammoclip[BG_FindClipForWeapon((weapon_t)weaponTable[pm->ps->weapon].akimboSideram)])
		{
			if (akimboFire)
			{
				addTime = 2 * GetAmmoTableData(pm->ps->weapon)->nextShotTime;
