...
void G_DropWeapon(gentity_t *ent, weapon_t weapon)
{
	...
	{
		ent2->delay = client->ps.ammo[BG_FindAmmoForWeapon((weapon_t)weaponTable[weapon].weapAlts)];
	}
	else
	{
		ent2->delay = 0;
...
int Pickup_Weapon(gentity_t *ent, gentity_t *other)
{
	...
	else
	{
		...
		if (G_CanPickupWeapon((weapon_t)ent->item->giTag, other))
		{
			...
			{
				primaryWeapon = G_GetPrimaryWeaponForClientSoldier((weapon_t)ent->item->giTag, other->client);
			}
			else
			{
...
gentity_t *LaunchItem(gitem_t *item, vec3_t origin, vec3_t velocity, int ownerNum)
{
	...
	if (item->giType == IT_TEAM)     // Special case for CTF flags
	{
		...
		{
			G_Script_ScriptEvent(level.gameManager, "trigger", (char *)(flag->item->giTag == PW_REDFLAG ? "allied_object_dropped" : "axis_object_dropped"));
		}
