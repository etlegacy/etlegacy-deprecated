	...
	if (Q_stricmpn(name, "skill", 5) == 0)
	{
		if (hasAmount)
		{
			...
			if (skill >= 0 && skill < SK_NUM_SKILLS)
			{
				...
				G_AddSkillPoints(ent, (skillType_t)skill, amount);
				G_DebugAddSkillPoints(ent, (skillType_t)skill, amount, "give skill");
			}
		}
		else
		{
			...
			for (i = 0; i < SK_NUM_SKILLS; i++)
			{
				...
				G_AddSkillPoints(ent, (skillType_t)i, amount);
				G_DebugAddSkillPoints(ent, (skillType_t)i, amount, "give skill");
			}
		}
		return;
	}
...
void Cmd_Follow_f(gentity_t *ent, unsigned int dwCommand, qboolean fValue)
{
	...
	if (i == -1)
	{
		...
		if (!TeamCount(ent - g_entities, (team_t)i))
		{
...
void Cmd_FollowCycle_f(gentity_t *ent, int dir, qboolean skipBots)
{
	...
	{
		SetTeam(ent, "s", qfalse, (weapon_t)-1, (weapon_t)-1, qfalse);
	}
