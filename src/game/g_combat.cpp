...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath)
{
	...
	if (attacker)
	{
		...
		killerName = (char *)((attacker->client) ? attacker->client->pers.netname : "<non-client>");
	}
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, vec3_t dir, vec3_t point, int damage, int dflags, int mod)
{
	...
	case ET_MOVER:
		...
		{
			if (!G_ModIsExplosive((meansOfDeath_t)mod))
			{
				return;
			}
	...
	if (knockback && targ->client)
	{
		...
		if (attacker && attacker->client && (targ->client->ps.groundEntityNum != ENTITYNUM_NONE || G_ModIsExplosive((meansOfDeath_t)mod)))
		{
...
qboolean CanDamage(gentity_t *targ, vec3_t origin)
{
	...
	VectorSet(offsetmins, -16.f, -16.f, -16.f);
	VectorSet(offsetmaxs, 16.f, 16.f, 16.f);
