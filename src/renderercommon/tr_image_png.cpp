...
static struct BufferedFile *ReadBufferedFile(const char *name)
{
	...
	BF = (struct BufferedFile *)ri.Z_Malloc(sizeof(struct BufferedFile));
	if (!BF)
	{
		return NULL;
	}
...
static uint32_t DecompressIDATs(struct BufferedFile *BF, uint8_t **Buffer)
{
	...
	*Buffer                = DecompressedData;
	...
	CompressedData = (uint8_t  *)ri.Z_Malloc(CompressedDataLength);
	if (!CompressedData)
	{
		return(-1);
	}
	...
	DecompressedData = (uint8_t *)ri.Z_Malloc(puffDestLen);
	if (!DecompressedData)
	{
		ri.Free(CompressedData);
...
void R_LoadPNG(const char *name, byte **pic, int *width, int *height, byte alphaByte)
{
	...
	*pic = NULL;
	...
	OutBuffer = (byte *)R_GetImageBuffer(IHDR_Width * IHDR_Height * Q3IMAGE_BYTESPERPIXEL, BUFFER_IMAGE, name);
	if (!OutBuffer)
	{
		ri.Free(DecompressedData);
		CloseBufferedFile(ThePNG);
