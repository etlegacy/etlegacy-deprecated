...
byte *RB_ReadZBuffer(int x, int y, int width, int height, int *padlen)
{
	...
	// Allocate a few more bytes so that we can choose an alignment we like
	buffer = (byte *)ri.Hunk_AllocateTempMemory(padwidth * height + packAlign - 1);

	bufstart = (byte *)PADP(( intptr_t ) buffer, packAlign);
...
void RB_TakeDepthshot(int x, int y, int width, int height, char *fileName)
{
	...
	buffer = (byte *)ri.Hunk_AllocateTempMemory(width * height * 3 + offset);
...
void R_Init(void)
{
	...
	ptr = (byte *)ri.Hunk_Alloc(sizeof(*backEndData) + sizeof(srfPoly_t) * r_maxpolys->integer + sizeof(polyVert_t) * r_maxpolyverts->integer, h_low);
